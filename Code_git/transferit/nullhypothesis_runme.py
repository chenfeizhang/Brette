...
data = np.load('%s/Series%s/spiketimelist.npy'%(foldername, ii))
dic = data.item()
...
for run in range(1,100+1): # null hypothesis test run number
  nspikes = 0
  STA_tmp = np.zeros(L)
  for k in range(len(spiketimelist)):
    ...
    sp2 = np.sort((spiketimelist[k]+random.uniform(0,T_s))%T_s) # Add a random number to all spike times and mod them by T_s.
